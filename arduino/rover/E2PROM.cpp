...
void E2PROM::begin_logging() {
	...
	if((!status) || (firstPage[0] != 0x4e)) {
		...
		if((!status) || (firstPage[0] != 0x4e)) {
			Serial.println("#EEPROM blank, formatting");			
			unsigned char newPage[7] = {0x4e, 0x4f, 0x56,0x00, 0x01, 0x00, 0x00};
			write_page(0, newPage);
		};
	};
	current_write_page = (newPage[3] << 8) + newPage[4];
...
unsigned char E2PROM::read_byte(int device, unsigned long address) {
	Wire.beginTransmission(DEVICE_ADDRESS(address >> 16,device));
	Wire.write((address >> 9) & 0xFF); //High byte
...
void E2PROM::write_byte(int device, unsigned long address, unsigned char data) {
	Wire.beginTransmission(DEVICE_ADDRESS(address >> 16,device));
	Wire.write((address >> 9) & 0xFF); //High address byte
...
bool E2PROM::read_page(unsigned int page, unsigned char *buffer) {
	...
	Wire.beginTransmission(DEVICE_ADDRESS((page >> 13) & 0x01,page >> 14));
	Wire.write((page >> 5) & 0xFF); //High address byte
	Wire.write((address * 8) & 0xFF); //Low address byte
	Wire.endTransmission();
	Wire.requestFrom(DEVICE_ADDRESS((page >> 13) & 0x01,page >> 14),8);
	while((Wire.available()) && (byteCount < 8)) {
		page[byteCount] = Wire.read();
		if(byteCount < 7) buffer[byteCount] = page[byteCount];
		byteCount++;
	};
	if(byteCount != 8) {
		return false;
...
void E2PROM::write_page(unsigned int page, unsigned char *data) {
	...
	for(int i = 0;i < 7;i++) {
		tmpsum += (7 - i) * data[i];
		write_byte(page >> 14, (page & 0x4000) + i,data[i]);
	};
	unsigned char checksum;
	checksum = tmpsum % 0xFF;
	write_byte(page >> 14, (page & 0x4000) + 7,checksum);
};

void E2PROM::write_next_page(unsigned char *data) {
	write_page(data,current_write_page);
	current_write_page++;
};
